...
CREATE TABLE `rearrangement` (
  ...
  `strand`  char(1) DEFAULT NULL,
  `status` varchar(20) DEFAULT NULL,
  `reference_strain_id` int(11) unsigned DEFAULT NULL,
  `species_id` int(11) unsigned DEFAULT NULL,
  `laboratory_id` int(11) unsigned DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE `atomized_genotype` (
  ...
  PRIMARY KEY (`id`),
  KEY `atomized_genotype_strain_id_fk` (`strain_id`),
  ...
  CONSTRAINT `atomized_genotype_strain_id_fk` FOREIGN KEY (`strain_id`) REFERENCES `strain` (`id`),
