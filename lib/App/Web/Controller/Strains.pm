...
sub all_strains :Path ('/all_strains') :Args(0) {
    my ($self,$c) = @_;
    $c->detach('all_species'),['all']);   # TH: what does all mean in this context?
}

sub recently_added :Path ('/recently_added') :Args(0) {
    my ($self,$c) = @_;
    $c->detach('recently_added'),['all']);   # TH: what does all mean in this context?
}

sub wild_strains :Path ('/wild_strains') :Args(0) {
    my ($self,$c) = @_;
    $c->detach('wild_strains'),['all']);   # TH: what does all mean in this context?
}

sub non_celegans_strains :Path ('/non_celegans_strains') :Args(0) {
    my ($self,$c) = @_;
    $c->detach('non_celegans_strains'),['all']);   # TH: what does all mean in this context?
}
